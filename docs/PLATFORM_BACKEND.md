...
| PostgreSQL schema | `open_payment` |
| Sentry organization | `hillstreet` |
| Sentry project | `open-payment` |
| GitHub repository | `hillstreet-ph/open-payment` |
| Container image | `${DOCKERHUB_USERNAME}/open-payment` |
...
7. Supabase migrations must target schema `open_payment` and must be reviewed before production execution.
